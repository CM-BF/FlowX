...
class LRP_gamma(GradientAttribution):

    def __init__(self, model: Module, gamma_list: list) -> None:
        r"""
        Args:
        ...
        # setting your gamma outside this explainer
        self.gamma = gamma_list
        self.saved_gamma = copy.deepcopy(gamma_list)

    def renew_gamma(self):
        self.gamma = copy.deepcopy(self.saved_gamma)
        return
    ...
    def _backward_hook(
        ...
        # print(type(module))
        print('here backward hook')
        # print(grad_input[0].shape, grad_input[1].shape, grad_input[2].shape, grad_output)
        if self._is_non_linear(module):
            multipliers = tuple(
            ...
            multipliers = tuple(
                linear(module, module.input, module.output, grad_input, grad_output, gamma=self.gamma)
            )
            # self.gamma.pop()
            print(f'gamma: {self.gamma}')
        else:
            raise Exception("Backward hook should not be registered on a module "
                            "that is not a non-linear and not a linear function.")
        # print(multipliers[0].shape, multipliers[1].shape, multipliers[2].shape)
    ...
    def _can_register_hook(self, module: Module) -> bool:
        ...
        # --- First, adding general linear into hook register queue ---
        return not (
            "nn.modules.container" in module_fullname
            or has_already_hooks
            or (not self._is_non_linear(module) and not hasattr(module, 'weight'))
        )
...
def nonlinear(
    ...
    new_grad_inp[0] = grad_output[0] # ignore ReLU
...
def linear(
    ...
    r"""
    Note: Ignore biases.
    grad_input: grad_input [ / w * w^ ]_detached
    grad_output: (dLoss / dlayer_out)
    https://github.com/pytorch/pytorch/issues/12331
    """
    eps = 1e-20
    w_up = module.weight + gamma[-1] * module.weight.relu()
    print('here linear')

    new_grad_inp = list(grad_input)
    print(module)
    print(inputs.shape, outputs.shape)
    print(new_grad_inp[1].shape)
    # supported non-linear modules take only single tensor as input hence accessing
    # only the first element in `grad_input` and `grad_output`
    print(len(new_grad_inp))
    print(new_grad_inp[0].shape, new_grad_inp[1].shape)
    print(new_grad_inp[1].shape, grad_output[0].shape, module.weight.shape, w_up.shape)
    new_grad_inp[1] = grad_output[0] @ (w_up).detach()
    print('after', new_grad_inp[1].shape, grad_output[0].shape, module.weight.shape, w_up.shape)

    # If the module is invalid, save the newly computed gradients
    # The original_grad_input will be overridden later in the Tensor hook
    if module.is_invalid:
        module.saved_grad = new_grad_inp[1]
    print('finish linear')
    return new_grad_inp
