...
class Node_Explainer:
    ...
    def explain(self, node_idx, subset, device, num_samples = 100, top_node = None, p_threshold = 0.05, pred_threshold = 0.1):
        ...
        neighbors = subset.cpu().numpy()
        
        if (node_idx not in neighbors):
            neighbors = np.append(neighbors, node_idx)
        
        X_torch = self.X #torch.tensor([self.X], dtype=torch.float)
        A_torch = self.A #torch.tensor([self.A], dtype=torch.float)
        pred_torch = self.model.forward(X_torch, A_torch).cpu()
        ...
        for iteration in range(num_samples):
            ...
            X_perturb = self.X.cpu().numpy().copy()
            sample = []
            ...
            X_perturb_torch =  torch.tensor([X_perturb], dtype=torch.float).to(device)[0]
            pred_perturb_torch = self.model.forward(X_perturb_torch, A_torch).cpu()
            soft_pred_perturb = np.asarray([softmax(np.asarray(pred_perturb_torch[node_].data)) for node_ in range(self.X.shape[0])])
        ...
        return pgm_nodes, data, pgm_stats, p_values
