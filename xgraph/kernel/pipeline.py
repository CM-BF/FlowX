...
from xgraph.kernel import val, test, init, set_train_utils
from xgraph.kernel.utils import save_epoch, argus_parse, detail_results
from xgraph.kernel.explain import XCollector, sample_explain
...
def main():
    ...
    if args['common'].task == 'train':
        ...
        explain_tik.record(stream=torch.cuda.current_stream(args['common'].device_idx))
        ...
        explain_tok.record(stream=torch.cuda.current_stream(args['common'].device_idx))
