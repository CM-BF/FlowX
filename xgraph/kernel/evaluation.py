...
"""
import torch
from xgraph import val_args, test_args, data_args
from torch_geometric.data.batch import Batch
from torch.utils.data import DataLoader
import torch.nn.functional as F
from xgraph import logger
...
def val(model: torch.nn.Module, loader: DataLoader):
    with torch.no_grad():
        model.eval()
        ...
        # ROC_AUC/Accuracy score calculate ---------------------------------
        stat['score'] = np.nanmean(Metric.score_func(target_all, pred_all))


        print(f'#IN#Validation {Metric.score_name}: {stat["score"]:.4f}\n'
              f'Validation Loss: {stat["loss"]:.4f}')
