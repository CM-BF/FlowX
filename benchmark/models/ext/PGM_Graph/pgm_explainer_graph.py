...
class Graph_Explainer:
    ...
    def perturb_features_on_node(self, feature_matrix, node_idx, random = 0):
        ...
        X_perturb = feature_matrix.clone().detach()
        perturb_array = X_perturb[node_idx].clone().detach()
        epsilon = 0.05*np.max(self.x.cpu().numpy(), axis = 0)
                    ...
                    if i in self.perturb_feature_list:
                        if self.perturb_mode == "mean":
                            ...
                            if perturb_array[i] < 0:
                                perturb_array[i] = 0
                            elif perturb_array[i] > np.max(self.x, axis = 0)[i]:
                                perturb_array[i] = np.max(self.x, axis = 0)[i]
    ...
    def batch_perturb_features_on_node(self, num_samples, index_to_perturb,
        ...
        soft_pred = np.asarray(softmax(np.asarray(pred_torch[0].cpu().data)))
        pred_label = np.argmax(soft_pred)
        num_nodes = self.x.shape[0]
        Samples = [] 
        for iteration in range(num_samples):
            X_perturb = self.x.clone().detach()
            sample = []
            ...
            soft_pred_perturb = np.asarray(softmax(np.asarray(pred_perturb_torch[0].cpu().data)))
...
#         Round 1
        ...
        candidate_nodes = range(num_nodes) # calculate all scores
