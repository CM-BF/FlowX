...
import os, sys
from benchmark.data.dataset_gen import SentiGraphDataset


def graph_sst2(name):
    data_args.dataset_type = 'nlp'
    data_args.model_level = 'graph'

    dataset = SentiGraphDataset(root=os.path.join(ROOT_DIR, 'benchmark', 'datasets'),
                                name='Graph-SST2')
